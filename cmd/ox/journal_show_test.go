...
// runJournalShowInProc executes journalShowCmd in-process with a fresh
// flag state and positional IDs. Mirrors runJournalListInProc so both
...
// returned error for *journalExitError.
//
// We rebuild the cobra.Command each call because cobra keeps flag
// state on the command, which would bleed between subtests.
func runJournalShowInProc(t *testing.T, args ...string) (string, string, error) {
	t.Helper()

	journalShowFlagSet = journalShowFlags{}
	...
	f := cmd.Flags()
	f.StringVar(&journalShowFlagSet.Team, "team", "", "")
	f.StringVar(&journalShowFlagSet.Format, "format", "json", "")
...
// TestJournalShow_InProc_ExactStemJSON — happy path: full stem match
...
// set type=journal_show.
func TestJournalShow_InProc_ExactStemJSON(t *testing.T) {
	...
	out, stderrStr, err := runJournalShowInProc(t, "2026-04-12-019c8a3f", "--format=json")
	if err != nil {
	...
	if env.Type != "journal_show" {
		t.Fatalf("type=%q, want journal_show", env.Type)
	}
...
// TestJournalShow_InProc_BareDateMultiRow — bare date expands to the
...
func TestJournalShow_InProc_BareDateMultiRow(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, stderrStr, err := runJournalShowInProc(t, "2026-04-12", "--format=json")
	if err != nil {
		t.Fatalf("show err=%v stdout=%s stderr=%s", err, out, stderrStr)
	}
...
// TestJournalShow_InProc_PartialSuccess — mix of ok + not_found IDs
...
func TestJournalShow_InProc_PartialSuccess(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runJournalShowInProc(t,
...
// TestJournalShow_InProc_AllFailed — every requested ID is a miss →
...
func TestJournalShow_InProc_AllFailed(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	// No fixture files — every ID must miss.
	out, _, err := runJournalShowInProc(t, "2026-04-12-deadbeef", "--format=json")
	if err == nil {
...
// TestJournalShow_InProc_AmbiguousExit1 — single ambiguous ID is an
...
// exit 0.
func TestJournalShow_InProc_AmbiguousExit1(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runJournalShowInProc(t, "019c8a3f", "--format=json")
	if err == nil {
...
// TestJournalShow_InProc_ContentFormatStripsEnvelope — --format=content
...
// between entries.
func TestJournalShow_InProc_ContentFormatStripsEnvelope(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runJournalShowInProc(t, "2026-04-12", "--format=content")
	if err != nil {
		t.Fatalf("show err=%v stdout=%s", err, out)
	}
...
// TestJournalShow_InProc_UsageErrorMissingArgs — no positional IDs is
...
func TestJournalShow_InProc_UsageErrorMissingArgs(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runJournalShowInProc(t, "--format=json")
	if err == nil {
...
// TestJournalShow_InProc_LatestFlagRejectedByCobra — --latest must
// NOT be a registered flag. Cobra rejects unknown flags at parse time
// with its own error, before runJournalShow sees anything. Failure
...
func TestJournalShow_InProc_LatestFlagRejectedByCobra(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	_, _, err := runJournalShowInProc(t, "2026-04-12", "--latest")
	if err == nil {
	...
	if errors.As(err, new(*journalExitError)) {
		t.Fatalf("--latest reached runJournalShow instead of cobra's unknown-flag path: %v", err)
	}
}

// TestJournalShow_InProc_TextFormat pins the --format=text shape
...
// "entry " token.
func TestJournalShow_InProc_TextFormat(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runJournalShowInProc(t, "2026-04-12-019c8a3f", "--format=text")
	if err != nil {
...
// TestJournalShow_InProc_ContentFormatAllFailedFallsBackToJSON —
...
// journalShowErrorFormat("content") → "json" mapping in the command
// layer.
//
...
// journal_list anti-pattern lesson (stored in bd memory as
// cobra-json-envelope-anti-pattern-when-a-command).
func TestJournalShow_InProc_ContentFormatAllFailedFallsBackToJSON(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	// No fixture files — every ID must miss.
	out, _, err := runJournalShowInProc(t, "2026-04-12-deadbeef", "--format=content")
	if err == nil {
