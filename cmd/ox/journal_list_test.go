...
func stageJournalListProject(t *testing.T) (string, string) {
	...
	if err := config.SaveProjectConfig(projectRoot, &config.ProjectConfig{
		ProjectID:   "proj_journal_list",
		WorkspaceID: "ws_journal_list",
		TeamID:      "team_journal_list",
...
// runJournalListInProc executes journalListCmd in-process with a fresh
// flag state (important: journalListFlagSet is package-scoped, so a
...
func runJournalListInProc(t *testing.T, args ...string) (string, string, error) {
	...
	f := cmd.Flags()
	f.StringVar(&journalListFlagSet.Since, "since", "", "")
	f.StringVar(&journalListFlagSet.Until, "until", "", "")
	f.StringVar(&journalListFlagSet.TZ, "tz", "", "")
	f.StringVar(&journalListFlagSet.Layer, "layer", "auto", "")
	f.StringVar(&journalListFlagSet.Team, "team", "", "")
	f.BoolVar(&journalListFlagSet.AllTeams, "all-teams", false, "")
	f.IntVar(&journalListFlagSet.Limit, "limit", 0, "")
	f.StringVar(&journalListFlagSet.Format, "format", "json", "")
...
// TestJournalList_InProc_DailyJSON — happy-path daily list through the
...
func TestJournalList_InProc_DailyJSON(t *testing.T) {
	...
	out, stderrStr, err := runJournalListInProc(t,
	...
	if env.Type != "journal_list" {
		t.Fatalf("type = %q, want journal_list", env.Type)
	}
	if env.Data == nil {
...
// TestJournalList_InProc_AutoResolvesWeeklyAndMonthly — verifies
...
// and every `ox journal list` call hits the daily branch regardless of
// window width.
func TestJournalList_InProc_AutoResolvesWeeklyAndMonthly(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	t.Run("weekly window", func(t *testing.T) {
		out, _, err := runJournalListInProc(t,
...
// TestJournalList_InProc_UsageErrorExitCode — verifies a malformed
// --since value funnels through newJournalUsageExit and surfaces on
// stdout as a JSON error envelope with code=usage_error, while also
// returning a *journalExitError with ExitCode=2. The exit-code plumbing
...
func TestJournalList_InProc_UsageErrorExitCode(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runJournalListInProc(t, "--since=not-a-duration", "--format=json")
	if err == nil {
		t.Fatalf("want usage error, got nil (stdout=%s)", out)
	}
	var jexit *journalExitError
	if !asJournalExitError(err, &jexit) {
		t.Fatalf("want *journalExitError, got %T: %v", err, err)
	}
	if jexit.ExitCode != 2 {
...
// TestJournalList_InProc_TeamAllTeamsConflict — passing --team and
...
func TestJournalList_InProc_TeamAllTeamsConflict(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runJournalListInProc(t,
		"--since=2026-04-12", "--until=2026-04-12",
		"--team=journal-list-test", "--all-teams", "--format=json")
	if err == nil {
		t.Fatalf("want usage error, got nil (stdout=%s)", out)
	}
	var jexit *journalExitError
...
// TestJournalList_InProc_MalformedFormatFlag — a rejected --format
...
func TestJournalList_InProc_MalformedFormatFlag(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)
...
// TestJournalList_InProc_LegacyPrefixTrusted — a daily file whose UUID7
...
func TestJournalList_InProc_LegacyPrefixTrusted(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
...
// TestJournalList_InProc_TextFormatGolden — the --format=text renderer
...
//
// Failure prevented: renderJournalListText regresses in a way that
// breaks human-scanned output or downstream shell pipelines that grep
// the line format.
func TestJournalList_InProc_TextFormatGolden(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	want := "2026-04-12  daily    2026-04-12-019c8a3f  team=journal-list-test  facts=0  cites=1\n" +
...
// TestJournalList_InProc_AllTeamsMerged — --all-teams merges entries
...
func TestJournalList_InProc_AllTeamsMerged(t *testing.T) {
	projectRoot := t.TempDir()
	teamA := t.TempDir()
	teamB := t.TempDir()
...
// TestJournalList_InProc_WindowRoundingReported — a --since=6h query
// funnels through resolveJournalWindow + day-floor/ceil math. The
...
// the end-to-end equivalent of TestJournalWindowRounding.
//
...
func TestJournalList_InProc_WindowRoundingReported(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runJournalListInProc(t, "--since=6h", "--format=json")
	if err != nil {
		t.Fatalf("list: %v\n%s", err, out)
	}
	var env journalEnvelope
...
// asJournalExitError unwraps a *journalExitError via errors.As so a
...
// uniformly.
func asJournalExitError(err error, target **journalExitError) bool {
