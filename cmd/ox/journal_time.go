...
// journalUsageError is the typed usage error the journal command layer
...
// string. Every fatal user-input failure in the journal read surface
...
func newJournalUsageError(msg string) *journalUsageError {
	return &journalUsageError{Code: "usage_error", Message: msg}
}

// resolveJournalWindow parses --since, --until, and --tz flag values
// into a UTC [since, until] window. Returns a *journalUsageError for
...
// deterministically.
//
// Grammar (from journal-read-plan.md §3 Unit 3):
//
...
func resolveJournalWindow(sinceStr, untilStr, tzStr string, now time.Time) (time.Time, time.Time, error) {
	if sinceStr == "" {
