...
// journal_read_e2e_harness_test.go — hermetic E2E harness for the ox
// journal read surface. Owns the types, the per-test setup function,
...
// journal_read_e2e_fixtures_test.go. The sanity tests that prove the
// whole thing wires up live in journal_read_e2e_test.go. The JR-*
// skeleton test cases live in the list/show/since files.
//
// See docs/ai/specs/journal-read-test-plan.md §1.
...
// journalE2E is the bundle of paths and environment that a JR-* test uses
...
type journalE2E struct {
	...
	// by setupJournalE2E from its parameter and never mutated afterward.
	now time.Time
}
...
// findJournalProjectRoot walks up from cwd until it finds a go.mod whose
...
func findJournalProjectRoot(t *testing.T) string {
	t.Helper()
	dir, err := os.Getwd()
...
// setupJournalE2E builds a per-test harness: fresh ox binary, fresh
...
// runJournalOx to invoke the binary against it, and call stage* helpers
...
// docs/ai/specs/journal-read-test-plan.md §4.3). The value is stored on
// the returned *journalE2E so time-dependent helpers (writeFakeAuth
// today, future ones) never call time.Now() independently.
func setupJournalE2E(t *testing.T, now time.Time) *journalE2E {
	t.Helper()

	projectRoot := findJournalProjectRoot(t)
	...
	initJournalGitRepo(t, workspace, home)
...
// semantics.
func addSecondaryTeam(t *testing.T, e2e *journalE2E, id, name, slug string) teamContext {
	t.Helper()
...
// e2e.Run(t, "journal", "list", ...) without recomputing env every
...
func (e *journalE2E) Run(t *testing.T, args ...string) (string, int) {
	t.Helper()
...
// initJournalGitRepo initializes a workspace as a git repo with one commit.
// HOME is scoped so git cannot read the developer's ~/.gitconfig.
func initJournalGitRepo(t *testing.T, workspace, home string) {
	t.Helper()
...
// the memory subtree the journal reader expects. Returns a populated
