...
func outputAgentPrimeXML(cmd *cobra.Command, output agentPrimeOutput) (*prime.ContextBudget, error) {
	...
	if output.CodeDBAvailable {
		...
		sb.WriteString("Verb-mode (preferred when the intent matches one of these):\n")
		...
		sb.WriteString("Use Grep/Glob ONLY when: (a) exact-string match in a known file,\n")
		sb.WriteString("or (b) `ox code` returned 0 results and you suspect typo or stale index.\n")
