...
// runDistillHistorySinceInProc executes distillHistorySinceCmd in-process with a fresh
// flag state and positional args. Mirrors runDistillHistoryListInProc and
// runDistillHistoryShowInProc so all three command tests share the "reset
...
func runDistillHistorySinceInProc(t *testing.T, args ...string) (string, string, error) {
	t.Helper()

	distillHistorySinceFlagSet = distillHistorySinceFlags{}

	cmd := &cobra.Command{
		Use:           "since",
		RunE:          runDistillHistorySince,
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
// TestDistillHistorySince_InProc_HappyPathJSON — the composite returns two
// entries in chronological order with bodies parallel by index. The
// envelope type is distill_history_since, window is day-rounded, and the
...
func TestDistillHistorySince_InProc_HappyPathJSON(t *testing.T) {
	...
	out, stderrStr, err := runDistillHistorySinceInProc(t, "6h", "--format=json", "--layer=daily")
	if err != nil {
	...
	if env.Type != "distill_history_since" {
		t.Fatalf("type=%q, want distill_history_since", env.Type)
	}
	if env.Data == nil {
...
// TestDistillHistorySince_InProc_ContentFormatRichMarkers — the default
...
func TestDistillHistorySince_InProc_ContentFormatRichMarkers(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runDistillHistorySinceInProc(t, "6h", "--layer=daily")
	if err != nil {
...
// TestDistillHistorySince_InProc_ContentFormatGolden — pins the exact
...
func TestDistillHistorySince_InProc_ContentFormatGolden(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
...
// TestDistillHistorySince_InProc_EmptyWindowJSON — a window that catches
...
func TestDistillHistorySince_InProc_EmptyWindowJSON(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	// No fixtures — any duration yields zero entries.
	out, _, err := runDistillHistorySinceInProc(t, "1h", "--format=json", "--layer=daily")
	if err != nil {
		t.Fatalf("since err=%v stdout=%s", err, out)
	}
...
// TestDistillHistorySince_InProc_EmptyWindowContent — an empty window in the
...
func TestDistillHistorySince_InProc_EmptyWindowContent(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "1h", "--layer=daily")
	if err != nil {
		t.Fatalf("since err=%v stdout=%s", err, out)
	}
	if out != "" {
...
// TestDistillHistorySince_InProc_AllTeamsMerged — --all-teams merges entries
...
// the second team.
func TestDistillHistorySince_InProc_AllTeamsMerged(t *testing.T) {
	...
	out, _, err := runDistillHistorySinceInProc(t, "6h", "--all-teams", "--layer=daily")
	if err != nil {
		t.Fatalf("since err=%v stdout=%s", err, out)
	}
...
// TestDistillHistorySince_InProc_TeamAllTeamsConflict — combining --team and
// --all-teams is a usage error before any reader work happens. Mirrors
// the list test of the same shape.
func TestDistillHistorySince_InProc_TeamAllTeamsConflict(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "6h", "--team=foo", "--all-teams", "--format=json")
	if err == nil {
...
// TestDistillHistorySince_InProc_LimitRespected — --limit=1 caps the returned
...
// exists to relieve.
func TestDistillHistorySince_InProc_LimitRespected(t *testing.T) {
	projectRoot, teamRoot := stageJournalListProject(t)
	t.Chdir(projectRoot)
	...
	out, _, err := runDistillHistorySinceInProc(t, "6h", "--layer=daily", "--limit=1", "--format=json")
	if err != nil {
		t.Fatalf("since err=%v stdout=%s", err, out)
	}
	var env distillHistoryEnvelope
...
// TestDistillHistorySince_InProc_UsageErrorMissingDur — no positional
...
// write its envelope.
func TestDistillHistorySince_InProc_UsageErrorMissingDur(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "--format=json")
	if err == nil {
		t.Fatalf("want usage error, got nil stdout=%s", out)
	}
	var exit *distillHistoryExitError
...
// TestDistillHistorySince_InProc_UsageErrorInvalidDur — an un-parseable
// duration is a usage error (exit 2) with a JSON envelope on stdout
// whose message quotes the offending value.
func TestDistillHistorySince_InProc_UsageErrorInvalidDur(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "banana", "--format=json")
	if err == nil {
		t.Fatalf("want usage error, got nil stdout=%s", out)
	}
	var exit *distillHistoryExitError
...
// TestDistillHistorySince_InProc_UsageErrorNegativeDur — a negative duration
...
func TestDistillHistorySince_InProc_UsageErrorNegativeDur(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "--format=json", "--", "-5m")
	if err == nil {
		t.Fatalf("want usage error, got nil stdout=%s", out)
	}
	var exit *distillHistoryExitError
...
// TestDistillHistorySince_InProc_UsageErrorInvalidFormat — --format=yaml is
// not a registered format, so runDistillHistorySince must reject it up front
...
// line.
func TestDistillHistorySince_InProc_UsageErrorInvalidFormat(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	out, _, err := runDistillHistorySinceInProc(t, "6h", "--format=yaml")
	if err == nil {
		t.Fatalf("want usage error, got nil stdout=%s", out)
	}
	var exit *distillHistoryExitError
...
// TestDistillHistorySince_InProc_TzFlagRejectedByCobra — --tz is deliberately
...
// the unknown flag at parse time, before runDistillHistorySince sees
// anything.
//
...
func TestDistillHistorySince_InProc_TzFlagRejectedByCobra(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	_, _, err := runDistillHistorySinceInProc(t, "6h", "--tz=America/Los_Angeles")
	if err == nil {
	...
	if errors.As(err, new(*distillHistoryExitError)) {
		t.Fatalf("--tz reached runDistillHistorySince instead of cobra unknown-flag path: %v", err)
	}
}

// TestDistillHistorySince_InProc_LatestFlagRejectedByCobra — --latest is
// plan-banned on every distill history read command. Since must reject it at
// cobra parse time. Same rationale as the list/show tests of the same
// shape: no newest-snapshot shortcut.
func TestDistillHistorySince_InProc_LatestFlagRejectedByCobra(t *testing.T) {
	projectRoot, _ := stageJournalListProject(t)
	t.Chdir(projectRoot)

	_, _, err := runDistillHistorySinceInProc(t, "6h", "--latest")
	if err == nil {
		t.Fatalf("--latest must not be accepted")
	}
	if errors.As(err, new(*distillHistoryExitError)) {
		t.Fatalf("--latest reached runDistillHistorySince: %v", err)
	}
}
