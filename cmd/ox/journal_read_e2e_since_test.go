...
// journal_read_e2e_since_test.go — Batch E JR-* cases for `ox journal
// since`. The 3 cases that exercise the since composite (listEntries +
...
// (Since + ox journal since, commit d55ce32). Per plan §3.6 and
// journal_since.go:
//
...
//     tests in internal/journal/read/since_test.go.
//
...
func TestJournalRead_JR09_SinceContentFormat_ChronoOrder(t *testing.T) {
	...
	if strings.Contains(out, `"success"`) || strings.Contains(out, `"journal_since"`) || strings.Contains(out, `"entries"`) {
	...
	// (journal_since.go:231-232). This is the contract distinguishing since
	...
	// renderer emits (journal_since.go:237). Exactly one separator for two
...
func TestJournalRead_JR10_SinceJsonFormat_ParallelBodies(t *testing.T) {
	...
	if env.Type != "journal_since" {
		t.Fatalf("JR-10: type=%q want 'journal_since'", env.Type)
	}
	if env.Error != nil {
	...
	// (journal_envelope.go:60-65 rationalizes the pointer indirection).
	...
	if len(bodies) != len(env.Data.Entries) {
		t.Fatalf("JR-10: bodies len=%d entries len=%d — arrays must be parallel (journal_since.go contract)",
...
func TestJournalRead_JR16_SinceAllTeamsContent_ConcatenatedMarkers(t *testing.T) {
	...
	// The spec commits to id/layer/date in the since marker (journal_since.go:
