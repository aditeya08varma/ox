...
func TestWriteRetryUploadMeta_PreservesSummaryFailureFields(t *testing.T) {
	...
	_, err := writeRetryUploadMeta(sessionDir, t.TempDir(), retryOrphan(name), nil)
...
func TestWriteRetryUploadMeta_UpdatesOwnedFields(t *testing.T) {
	...
	_, err := writeRetryUploadMeta(sessionDir, t.TempDir(), retryOrphan(name), refs)
	require.NoError(t, err)
...
// TestWriteRetryUploadMeta_DoesNotRotateSessionID — a retry must be
// idempotent in identity, or conversation URLs circulated during the
// session stop resolving.
func TestWriteRetryUploadMeta_DoesNotRotateSessionID(t *testing.T) {
	sessionDir := t.TempDir()
