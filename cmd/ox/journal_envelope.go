...
// journalEnvelope is the `--format=json` success/error response every
// journal read command emits. It matches the agent-ux envelope contract
// documented in team-memory-journal.md §4.1–§4.2 (Success and Error
// envelope shapes) and §4.3 (Entry and Window object shapes).
//
// The type is shared across `ox journal list`, `ox journal show`, and
// `ox journal since` so that a caller can decode one struct regardless
...
//     cmd/ox/journal_time.go guarantees UTC on every instant that
...
type journalEnvelope struct {
	...
	Data      *journalEnvelopeData `json:"data,omitempty"`
...
// journalEnvelopeData carries the command-specific payload. Fields are
// a union across all three journal read commands; each command only
...
// journalEnvelopeEntry mirrors the Entry shape in spec §4.3. Fields are
...
type journalEnvelopeWindow struct {
	Since         string `json:"since"`
	Until         string `json:"until"`
	LayerResolved string `json:"layer_resolved,omitempty"`
}

// journalEnvelopeError mirrors the Error object in spec §4.2. code is a
...
// errors or not-found).
type journalEnvelopeError struct {
	Code      string `json:"code"`
	Message   string `json:"message"`
	Retryable bool   `json:"retryable"`
}

// journalExitError wraps an envelope with an explicit exit code so the
...
// → exit 1, success → exit 0 (and we do not construct a journalExitError
...
func (e *journalExitError) Error() string {
	...
	return "journal command failed"
}

// newJournalUsageExit wraps a *journalUsageError into an envelope +
// exit-code-2 journalExitError. The command layer's RunE funnels every
// usage-error return through this helper so the JSON shape stays
// consistent across commands.
func newJournalUsageExit(err *journalUsageError, elapsedMS int64) *journalExitError {
