...
// journal_read_e2e_show_test.go — Batch D JR-* cases for `ox journal
// show`. The 7 cases that exercise the show command. Harness, fixtures,
// and recipes live in journal_read_e2e_test.go; this file only adds the
// per-case test funcs.
//
// All 7 cases assert real envelope / stdout behavior against Unit 4
// (LoadEntries + ox journal show). Per plan §3 Unit 4, show is
...
func TestJournalRead_JR04_ShowBareDate_ReturnsAllSnapshots(t *testing.T) {
	...
	if env.Type != "journal_show" {
		t.Fatalf("JR-04: type=%q want %q", env.Type, "journal_show")
	}
	if env.Error != nil {
...
func TestJournalRead_JR05_ShowLatestRejected(t *testing.T) {
	...
	// cobra rejects unknown flags at parse time — BEFORE runJournalShow
	// runs — so this path does NOT emit a journalEnvelope at all. The
	...
	// Hard lock-in: none of the journalEnvelope shape markers may appear
	// on stdout. If a future change silently swallows unknown flags and
	// fell through to runJournalShow, a JR-04-shaped envelope would leak
	// out and this check would trip.
	if strings.Contains(out, `"success"`) || strings.Contains(out, `"journal_show"`) || strings.Contains(out, `"entries"`) {
...
func TestJournalRead_JR06_ShowShortUUID7Prefix(t *testing.T) {
	...
	if env.Type != "journal_show" {
		t.Fatalf("JR-06: type=%q want journal_show", env.Type)
	}
	if env.Error != nil {
...
func TestJournalRead_JR07_ShowAmbiguousPrefix_Errors(t *testing.T) {
	...
	// in journal_show.go, only if some row came back ok does exit 0
...
func TestJournalRead_JR08_ShowContentFormatStripsEnvelope(t *testing.T) {
	...
	if strings.Contains(out, `"success"`) || strings.Contains(out, `"entries"`) || strings.Contains(out, `"journal_show"`) {
		t.Fatalf("JR-08: envelope fields leaked into content stdout:\n%s", out)
	}
	// The per-entry marker comment is allowed on the first non-blank
	// line (renderJournalShowContent emits `<!-- entry: %s -->` before
...
func TestJournalRead_JR17_ShowJsonFormat_FullBody(t *testing.T) {
	...
	if env.Type != "journal_show" {
		t.Fatalf("JR-17: type=%q want journal_show", env.Type)
	}
	...
	// in internal/journal/read/show_test.go:185 so this e2e test will
...
func TestJournalRead_JR18_ShowBadFrontmatterNeighbor_SurvivesGoodID(t *testing.T) {
	...
	// internal/journal/read/list.go:342-372), so the bad .md is listed
