...
// journal_read_e2e_test.go — harness sanity tests for the ox journal
// read surface. This file holds the three "does the whole thing wire
// up?" tests that exercise setupJournalE2E + every fixture recipe +
// addSecondaryTeam. The harness types/setup live in
// journal_read_e2e_harness_test.go; the stagers and 12 recipes live in
// journal_read_e2e_fixtures_test.go; the per-case JR-* skeletons live
...
// (plan §4.3).
//
// See docs/ai/specs/journal-read-test-plan.md §1 and §3.
...
// TestJournalE2E_Recipes_Stage exercises every fixture recipe end-to-end
// through setupJournalE2E and asserts that:
...
func TestJournalE2E_Recipes_Stage(t *testing.T) {
	t.Parallel()
	...
	type recipeCase struct {
		...
		// the team root paths.
		run func(t *testing.T, e2e *journalE2E, now time.Time) stagedFixture
	...
	cases := []recipeCase{
		{
			name: "MinimalDaily",
			run: func(t *testing.T, e *journalE2E, now time.Time) stagedFixture {
...
// TestJournalE2E_HarnessShape proves that setupJournalE2E itself
...
// starts.
func TestJournalE2E_HarnessShape(t *testing.T) {
	t.Parallel()
...
// TestJournalE2E_SecondaryTeamIsolated proves that addSecondaryTeam
...
func TestJournalE2E_SecondaryTeamIsolated(t *testing.T) {
	t.Parallel()
