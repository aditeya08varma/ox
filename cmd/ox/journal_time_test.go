...
// is the only place the journal command layer touches time.Location,
...
//
// Failure prevented: any of the grammar rules in journal-read-plan.md
...
func TestResolveJournalWindow(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			since, until, err := resolveJournalWindow(tc.sinceStr, tc.untilStr, tc.tzStr, now)
			...
			if tc.wantErr {
				...
				if !errors.As(err, &uerr) {
					t.Fatalf("want *journalUsageError, got %T: %v", err, err)
				}
