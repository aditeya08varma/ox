...
// TestJournalCitationsBridge_NoFieldLoss — the memoryio.Citation struct
...
func TestJournalCitationsBridge_NoFieldLoss(t *testing.T) {
	t.Parallel()
	in := []citation{
...
// TestJournalCitationsBridge_CanonicalParserRoundTrip takes a daily file
...
// between distill_citations.go and internal/journal/memoryio/
...
func TestJournalCitationsBridge_CanonicalParserRoundTrip(t *testing.T) {
	t.Parallel()
	cites := []citation{
		{Num: 1, Label: "Discussion: 2026-03-10", URL: "https://example.com/d", Key: "https://example.com/d"},
		{Num: 2, Label: "sageox/ox#152", URL: "https://github.com/sageox/ox/pull/152", Key: "https://github.com/sageox/ox/pull/152"},
...
// drift.
func TestJournalFrontmatterBridge_CanonicalParserRoundTrip(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		in   string
