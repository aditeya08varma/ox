<!-- doc-audience: ai -->
# `ox journal` Read Commands — Implementation Plan

Status: plan
Owner: tooling-engineer
Scope: `ox journal list`, `ox journal show`, `ox journal since`
...
call — `ox journal since 24h --format=content` — that returns every
journal body in the window as one contiguous markdown stream.

**In scope.**

- New top-level `ox journal` Cobra namespace containing `list`, `show`,
  `since` as visible subcommands.
- A new internal package `internal/journal/read` (the "reader library")
...
The reader lives at `internal/journal/read`. Every read command in
...
- **Window semantics.**
  ...
  - **The effective window is what agents see.** `ListMeta` carries
    ...
    `ox journal since 6h` at 2026-04-12T15:00Z produces
...
```go
package read // import "github.com/sageox/ox/internal/journal/read"
...
### 2.3 Derived helpers (internal to `internal/journal/read`)
...
### Unit 1 — `internal/journal/read` skeleton + `ListEntries` on daily

**What changes.** Create `internal/journal/read/` with:
...
Plus: one file under `cmd/ox/` that stubs `journalCmd` and the three
...
- `internal/journal/read/read_integration_test.go`: spin up a
...
- No other package imports `internal/journal/read` yet.
- The `journalCmd` skeleton under `cmd/ox/` is registered but hidden
...
- `internal/journal/memoryio/` (new package) with:
...
`cmd/ox/journal_citations_bridge.go` that converts `citation` →
...
- `internal/journal/memoryio` is a new package with no dependents
...
### Unit 3 — `ListEntries` weekly + monthly, `--layer=auto`, and `ox journal list`

**What changes.**

- Extend `internal/journal/read/list.go` to handle `Layer=="weekly"`
...
- Implement the envelope types in a new file
  `cmd/ox/journal_envelope.go` shared by all three commands. These
...
- Table tests for `cmd/ox/journal_time.go` covering: relative
...
- Table tests for the `cmd/ox/journal_list.go` flag parser using a
...
- A new `cmd/ox/journal_list_test.go` that uses
  ...
  fixtures, runs `journalListCmd.Execute()` in-process with JSON
...
### Unit 4 — `LoadEntries`, ID resolution, and `ox journal show`

**What changes.**

- Implement `read.LoadEntries` in `internal/journal/read/show.go`.
...
### Unit 5 — `Since` composite + `ox journal since`
...
- Wire `cmd/ox/journal_since.go`:
  ...
  - Content envelope: bodies concatenated with `\n---\n` separator
    ...
    <d> -->`).
  - **Time resolution** reuses `cmd/ox/journal_time.go` from Unit 3:
...
- Flip `journalCmd.Hidden = false` and the three subcommands from
  hidden to visible.
...
- Flag-visibility test: `journalCmd.Hidden == false`.

**Integration test strategy.**

- Smoke test: `ox journal list --format=json` on a real seeded
  fixture, assert JSON decodes cleanly and `type == "journal_list"`.
...
|---|---|---|---|
| `cmd/ox/distill.go:1516` `parseDailySources` | Carved into `internal/journal/memoryio.ParseFrontmatterSources` (Unit 2). Original is kept, wrapper-imported. | Not touched by tz plan. | No conflict. Keeping the original in place lets tz-removal rebase cleanly; Unit 2's carve-out is additive. |
| `cmd/ox/distill_citations.go:575` `parseSourcesSection` | NOT moved. Bridge file `cmd/ox/journal_citations_bridge.go` depends on it (Unit 2). | Not touched by tz plan. | No conflict. |
