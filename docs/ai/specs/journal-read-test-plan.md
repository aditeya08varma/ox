<!-- doc-audience: ai -->
# `ox journal` Read Surface — E2E Test Plan
...
The feature under test: new read-only commands `ox journal list`,
`ox journal show`, `ox journal since`. See `team-memory-journal.md`
...
(`journal-read-plan.md`); they are explicitly **out of scope** for
this document.
...
3. **`ox distill` alias** (spec §6.1). The hidden alias is tested by
   existing `distill_test.go`. We assert only that `ox journal list`
   / `show` / `since` exist; we do not re-test `ox distill`.
4. **Concurrent-reader race conditions.** Two `ox journal list`
