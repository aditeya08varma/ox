...
1. ~~codedb decision tagging~~ — **shipped in v1 at the search layer**: `ox code search` results under decision paths carry `doc_type:"decision"`, and `--decisions` filters the RAW result set (pre-limit, so DR hits are never crowded out by code hits); `ox query --source=code/all` results are tagged the same way. Remaining phase-2 depth: index-time facet fields (status, id, dr_type) inside codedb's schema for structured decision queries.
