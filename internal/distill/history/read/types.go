// Package read is the read-only distill-history reader for memory/daily,
// memory/weekly, and memory/monthly under a team-context root. It is the
// sole entry point the three ox distill history commands (list, show, since) use
...
// composition hook ox distill history show uses to mean "every entry regardless
...
// Entry is one distill-history entry (one .md file on disk). The shape matches
