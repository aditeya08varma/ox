...
---
<!-- ox-hash: 5811f1b25fba ver: 0.11.1 -->
...
- **No minutiae.** Do not walk through single lines of code, signatures, or implementation trivia the reader doesn't need to make the call. Describe **how the system behaves and why the change matters**, not how each function is wired. Altitude over detail; zoom in only where a risk lives. **Minutiae is relocated, not deleted** — push exact `file:line` steps and code trivia to the collapsed "Implementation notes" appendix at the foot of the page (see the layout rules): essential to the implementing agent, invisible to the ten-minute approver.
...
- All of it **pure CSS + vanilla JS, keyboard-focusable, `file://`-safe** — no network beyond the CDN libraries the self-contained contract already allows (Mermaid, Google Fonts); interactions and reveals must never add further requests. Tie it back to the time contract: the diagram answers "how does it behave" at a glance; deeper time is spent *only* on the node the reader chooses to interrogate.
...
- Steps as a clean numbered list with file:line refs styled distinctly (teal, small) — these implementation steps belong in the **"Implementation notes"** appendix (see above), not the decision layer up top.
...
- **Only when it legitimately helped.** If `ox plan enrich --json` returned no badges and an empty `context[]` (an un-enriched plan), add NO SageOx credit — there is nothing to credit.
