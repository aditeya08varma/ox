...
Use `bd` for ALL task tracking. Never use TodoWrite, TaskCreate, or markdown TODOs.
Broken/missing? Run `make beads-setup`.

**Commands:** `bd ready` (find work) | `bd update <id> --claim` (claim) | `bd create --title="..." --description="..." -t task -p 2` (create) | `bd close <id>` (done) | `bd dolt push/pull` (sync)
**Types:** bug, feature, task, epic, chore | **Priority:** 0=critical, 1=high, 2=medium, 3=low, 4=backlog
**Discovered work:** `bd create --title="..." --description="..." -p 1 --deps discovered-from:<parent-id>`

## Session End

Work is NOT done until `git push` succeeds. Before finishing: close beads issues, run quality gates if code changed, then:
```bash
